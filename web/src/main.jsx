  ...
  return <main className="admin-main">
    <section className="admin-shell">
      ...
      <section className="admin-content">
        ...
        {activeTab === 'competition-form' && <form className="panel" onSubmit={save}><div className="row"><h1>{editing ? 'Edit competition' : 'Add competition'}</h1>{editing && <button type="button" className="secondary" onClick={() => { setEditing(null); setForm(empty); }}>Cancel edit</button>}</div><label>Title<input value={form.title} onChange={e => updateField('title', e.target.value)} required /></label><label>Slug<input value={form.slug} onChange={e => updateField('slug', e.target.value)} required /></label><label>Description<textarea value={form.description} onChange={e => updateField('description', e.target.value)} /></label><div className="two"><label>Price pence<input type="number" value={form.ticket_price_pence} onChange={e => updateField('ticket_price_pence', Number(e.target.value))} /></label><label>Max tickets<input type="number" value={form.max_tickets} onChange={e => updateField('max_tickets', Number(e.target.value))} /></label></div><section className="fund-calculator panel-lite"><div className="row"><div><h2>Prize fund calculator</h2><p className="muted">Show customers exactly where the money goes if this draw sells out.</p></div><button type="button" className="secondary" onClick={autoCalculateMargin}>Auto-calc margin</button></div><div className="two"><label>Prize value pence<input type="number" value={form.prize_value_pence || 0} onChange={e => updateField('prize_value_pence', Number(e.target.value))} /></label><label>Running costs pence<input type="number" value={form.running_cost_pence || 0} onChange={e => updateField('running_cost_pence', Number(e.target.value))} /></label></div><div className="two"><label>Prizetown margin pence<input type="number" value={form.prizetown_margin_pence || 0} onChange={e => updateField('prizetown_margin_pence', Number(e.target.value))} /></label><label>Community pot pence<input type="number" value={form.community_pot_pence || 0} onChange={e => updateField('community_pot_pence', Number(e.target.value))} /></label></div><label>Transparency note<textarea value={form.transparency_note || ''} onChange={e => updateField('transparency_note', e.target.value)} placeholder="Example: Figures are based on the draw selling out. Actual running costs may vary." /></label><div className="fund-preview"><div><strong>{money(adminFund.total)}</strong><span>Max ticket income</span></div><div><strong>{pct(adminFund.prize, adminFund.total)}%</strong><span>Prize</span></div><div><strong>{pct(adminFund.running, adminFund.total)}%</strong><span>Running costs</span></div><div><strong>{pct(adminFund.margin, adminFund.total)}%</strong><span>Prizetown margin</span></div>{adminFund.community > 0 && <div><strong>{pct(adminFund.community, adminFund.total)}%</strong><span>Community pot</span></div>}</div></section><div className="two"><label>Max per user<input type="number" value={form.max_per_user} onChange={e => updateField('max_per_user', Number(e.target.value))} /></label><label>Status<select value={form.status} onChange={e => updateField('status', e.target.value)}><option>draft</option><option>active</option><option>closed</option></select></label></div><div className="two"><label>Closing date<input type="datetime-local" value={form.closes_at || ''} onChange={e => updateField('closes_at', e.target.value)} /></label><label>Draw date<input type="datetime-local" value={form.draw_at || ''} onChange={e => updateField('draw_at', e.target.value)} /></label></div><div className="two"><label>Minimum age<input type="number" value={form.min_age || 18} onChange={e => updateField('min_age', Number(e.target.value))} /></label><label className="check-row"><input type="checkbox" checked={form.age_restricted !== false} onChange={e => updateField('age_restricted', e.target.checked)} /> <span>Age restricted</span></label></div><label>Question<input value={form.question} onChange={e => updateField('question', e.target.value)} placeholder="Example: What colour is the sky?" /></label><label>Correct answer<input value={form.answer} onChange={e => updateField('answer', e.target.value)} /></label><label>Free entry route<textarea value={form.free_entry_text} onChange={e => updateField('free_entry_text', e.target.value)} /></label><label>Competition rules<textarea value={form.rules_text || ''} onChange={e => updateField('rules_text', e.target.value)} /></label><label>Prize image<input type="file" accept="image/*" onChange={uploadFile} /></label>{form.image_url && <img className="preview" src={imageUrl(form.image_url)} alt="Preview" />}<button className="primary full"><Plus size={16} /> {editing ? 'Save changes' : 'Add competition'}</button></form>}
