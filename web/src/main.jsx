  ...
  return <main className="admin-main">
    <section className="admin-shell">
      ...
      <section className="admin-content">
        {activeTab === 'overview' && <div className="panel"><div className="row"><h1>Dashboard overview</h1><div className="row-actions"><button type="button" className="secondary" onClick={seedDemo}>Add starter sample competitions</button><button type="button" className="danger" onClick={removeStarterCompetitions}>Remove starter samples</button></div></div><p className="muted">Use the sample buttons to quickly add or clear demo competitions while testing the poster strip, checkout and draw flow.</p><div className="stat-grid"><div><strong>{competitions.length}</strong><span>Total competitions</span></div><div><strong>{liveCount}</strong><span>Live competitions</span></div><div><strong>{totalTickets}</strong><span>Tickets allocated</span></div><div><strong>{money(revenue)}</strong><span>Test order value</span></div><div><strong>{instantClaimed}/{instantWins.length}</strong><span>Instant wins claimed</span></div></div><div className="admin-split"><div><h2>Recent orders</h2>{orders.slice(0, 8).map(o => <div className="list-row entry-row" key={o.id}><div><strong>Order #{o.id}</strong><p>{o.customer_email}  -  {money(o.total_pence)}  -  {o.entry_count} entries  -  {o.status}</p></div></div>)}</div><div><h2>Recent entries</h2>{entries.slice(0, 8).map(e => <div className="list-row entry-row" key={e.id}><div><strong>{e.competition_title}</strong><p>{e.customer_email}  -  ticket #{e.ticket_number}  -  {e.payment_status}</p></div></div>)}</div></div></div>}
