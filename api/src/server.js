...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'Prizetown API', version: 'v150' }));
app.get('/admin/system-check', auth('admin'), async (_req, res) => {
  ...
  add('ok', 'API version', 'Prizetown API is running.', { version: 'v150' });
...
initDb()
  .then(() => app.listen(port, () => console.log(`Prizetown API running on ${port} (v150 complete admin help guide)`)))
  .catch((err) => {
