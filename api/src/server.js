...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'Prizetown API', version: 'v305' }));
...
async function getGoogleDriveClient() {
  ...
  return { drive: google.drive({ version: 'v305', auth: authClient }), folderId };
}
...
app.get('/admin/system-check', auth('admin'), async (_req, res) => {
  ...
  add('ok', 'API version', 'Prizetown API is running.', { version: 'v305' });
