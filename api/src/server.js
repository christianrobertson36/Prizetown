...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'Prizetown API', version: 'v113' }));
app.get('/admin/system-check', auth('admin'), async (_req, res) => {
  ...
  add('ok', 'API version', 'Prizetown API v87 is running.', { version: 'v113' });
...
initDb()
  .then(() => app.listen(port, () => console.log(`Prizetown API running on ${port} (v113 topbar competitions hotfix)`)))
  .catch((err) => {
