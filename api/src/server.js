...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'Prizetown API', version: 'v306' }));
...
async function getGoogleDriveClient() {
  ...
  return { drive: google.drive({ version: 'v306', auth: authClient }), folderId };
}
...
app.get('/admin/system-check', auth('admin'), async (_req, res) => {
  ...
  add('ok', 'API version', 'Prizetown API is running.', { version: 'v306' });
