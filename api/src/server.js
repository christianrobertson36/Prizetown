...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'Prizetown API', version: 'v262' }));
  ...
  try {
    ...
    const handover = {
      ...
      current_expected_tags: {
        api: 'ghcr.io/christianrobertson36/prizetown-api:v262',
        web: 'ghcr.io/christianrobertson36/prizetown-web:v262'
      },
      google_drive: {
        folder_id_configured: Boolean(folderId),
        credentials_configured: Boolean(serviceAccountJson || credentialsFile),
...
function buildEnvironmentChecklist() {
  ...
  return {
    ...
    checks: [
      ...
      { name: 'Fixed API tag expected', ok: true, expected: 'ghcr.io/christianrobertson36/prizetown-api:v262' },
      { name: 'Fixed web tag expected', ok: true, expected: 'ghcr.io/christianrobertson36/prizetown-web:v262' }
    ],
    secret_policy: [
...
app.get('/admin/system-check', auth('admin'), async (_req, res) => {
  ...
  add('ok', 'API version', 'Prizetown API is running.', { version: 'v262' });
...
initDb()
  .then(() => app.listen(port, () => console.log(`Prizetown API running on ${port} (v262 backup tools cleanup speed)`)))
  .catch((err) => {
