...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'Prizetown API', version: 'v266' }));
  ...
  try {
    ...
    const handover = {
      ...
      current_expected_tags: {
        api: 'ghcr.io/christianrobertson36/prizetown-api:v266',
        web: 'ghcr.io/christianrobertson36/prizetown-web:v266'
      },
      google_drive: {
        folder_id_configured: Boolean(folderId),
        credentials_configured: Boolean(serviceAccountJson || credentialsFile),
...
function buildEnvironmentChecklist() {
  ...
  return {
    ...
    checks: [
      ...
      { name: 'Fixed API tag expected', ok: true, expected: 'ghcr.io/christianrobertson36/prizetown-api:v266' },
      { name: 'Fixed web tag expected', ok: true, expected: 'ghcr.io/christianrobertson36/prizetown-web:v266' }
    ],
    secret_policy: [
...
app.get('/admin/system-check', auth('admin'), async (_req, res) => {
  ...
  add('ok', 'API version', 'Prizetown API is running.', { version: 'v266' });
...
initDb()
  .then(() => app.listen(port, () => console.log(`Prizetown API running on ${port} (v266 automation control centre)`)))
  .catch((err) => {
