...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'Prizetown API', version: 'v285' }));
  ...
  try {
    ...
    const handover = {
      ...
      current_expected_tags: {
        api: 'ghcr.io/christianrobertson36/prizetown-api:v285',
        web: 'ghcr.io/christianrobertson36/prizetown-web:v285'
      },
      google_drive: {
        folder_id_configured: Boolean(folderId),
        credentials_configured: Boolean(serviceAccountJson || credentialsFile),
...
function buildEnvironmentChecklist() {
  ...
  return {
    ...
    checks: [
      ...
      { name: 'Fixed API tag expected', ok: true, expected: 'ghcr.io/christianrobertson36/prizetown-api:v285' },
      { name: 'Fixed web tag expected', ok: true, expected: 'ghcr.io/christianrobertson36/prizetown-web:v285' }
    ],
    secret_policy: [
...
app.get('/admin/system-check', auth('admin'), async (_req, res) => {
  ...
  add('ok', 'API version', 'Prizetown API is running.', { version: 'v285' });
...
initDb()
  .then(() => app.listen(port, () => console.log(`Prizetown API running on ${port} (v285 spinner preview gallery)`)))
  .catch((err) => {
